	...
	if(($_SERVER['REQUEST_METHOD'] == 'POST') && isset($_POST['comprar'])) {
		...
		if(isset($_SESSION['cesta'])){
			...
			if(isset($cestaRecupe[$codigoProducto])){
				//compruebo cuanto stock tengo del producto 
				$cantidadStock = cantidadStock ($codigoProducto);
				//Simulo el incremento en cesta
				$productoIncrement = $cestaRecupe[$codigoProducto]['cantidadProducto']++;
				//Si el incremento de cantidad en cesta supera el stock no guardo
				if($productoIncrement<$cantidadStock){
					//vuelvo a guardar en la session la cesta recu
					$_SESSION['cesta']=$cestaRecupe;
					Header('Location:'.$_SERVER['PHP_SELF']."?".$_SERVER['QUERY_STRING']);
				}else{
					echo "no puede añadir más productos a la cesta por falta de stock";
				}
